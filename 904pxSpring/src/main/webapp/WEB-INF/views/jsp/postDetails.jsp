...
<div class="row" style="padding-left: 40px;">
	...
	<c:if test="${not empty sessionScope.user_id}">
		<div class="card my-4">
			...
			<div class="card-body">
					<div class="form-group">
						<input id="text" name='text' style="height:50px; width:400px;" placeholder="Enter comment:" />
					</div>
		...
		<div class="media mb-4">
			...
			<h4><a href="/904px/profile/${comment.username}">${comment.username}</a></h4>
	...
	<div class="col-md-4">
		<div class="card my-4">
			<p>
				<b><i>Likes: ${post.likes} Views: ${post.views}</i></b>
			</p>
			<p class="lead">
			<c:if test='${post.description != "" }'>
			<p>	<b>Description: ${post.description}</b> </p>
			</c:if>
			<c:if test='${post.category != "" }'>
			<p>	<b>Category: ${post.category}</b> </p>
			</c:if>
			<c:if test='${post.cameraModel != "" }'>
			<p>	<b>Camera Model: ${post.cameraModel}</b></p>
			</c:if>
				<c:if test="${post.nsfw}">
			...
			<blockquote class="blockquote">
				...
				<c:if test='${post.imageCharacteristics.focalLength != "" }'>
				<p>	<b>Focal Length: ${post.imageCharacteristics.focalLength}</b></p>
				</c:if>
				<c:if test='${post.imageCharacteristics.fNumber != "" }'>
				<p>	<b>F Number: ${post.imageCharacteristics.fNumber}</b> </p>
				</c:if>
				<c:if test='${post.imageCharacteristics.exposureTime != "" }'>
				<p>	<b>Exposure Time: ${post.imageCharacteristics.exposureTime}</b> </p>
				</c:if>
				<c:if test='${post.imageCharacteristics.isoSpeedRatings != "" }'>
				<p> <b>	Iso Speed Ratings: ${post.imageCharacteristics.isoSpeedRatings}</b> </p>
				</c:if>
